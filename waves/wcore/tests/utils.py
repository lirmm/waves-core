...
class TestJobWorkflowMixin(object):
    def run_job_workflow(self, job):
        ...
        job.run_prepare()
        time.sleep(3)
        self.assertTrue(job.status == waves.wcore.adaptors.const.JOB_PREPARED,
                        "Wrong job status %s (expected %s) " % (job.status, waves.wcore.adaptors.const.JOB_PREPARED))
        job.run_launch()
        time.sleep(3)
        logger.debug('Remote Job ID %s', job.remote_job_id)
        self.assertTrue(job.status == waves.wcore.adaptors.const.JOB_QUEUED,
                        "Wrong job status %s (expected %s) " % (job.status, waves.wcore.adaptors.const.JOB_QUEUED))
        for ix in range(100):
        ...
        if job.status in (waves.wcore.adaptors.const.JOB_COMPLETED, waves.wcore.adaptors.const.JOB_TERMINATED):
            ...
            self.assertTrue(job.status == waves.wcore.adaptors.const.JOB_TERMINATED)
        else:
