...
from waves.wcore.models import get_service_model, Job, JobInput, JobOutput, Runner, AdaptorInitParam, \
    get_submission_model, ServiceRunParam
...
class TestJobWorkflowMixin(TestCase):
    ...
    def run_job_workflow(self, job):
        ...
        if job.status in (const.JOB_COMPLETED, const.JOB_TERMINATED):
            ...
            self.assertTrue(job.status == const.JOB_TERMINATED)
            job.delete()
        else:
