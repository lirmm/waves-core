...
class JobHistoryDetailSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'url': {'view_name': 'waves_api_v1:waves-jobs-detail', 'lookup_field': 'slug'}
        }
...
class JobOutputSerializer(serializers.ModelSerializer):
    ...
    def get_download_url(self, obj):
        ...
        return "%s?export=1" % reverse(viewname='waves_api_v1:waves-job-output', request=self.context['request'],
...
class JobSerializer(DynamicFieldsModelSerializer, serializers.HyperlinkedModelSerializer):
    ...
    service = serializers.HyperlinkedRelatedField(many=False, read_only=False,
                                                  view_name='waves_api_v1:waves-services-detail',
    ...
    def get_history(self, obj):
        ...
        return reverse(viewname='waves_api_v1:waves-jobs-history', request=self.context['request'],
                       kwargs={'slug': obj.slug})

    def get_outputs(self, obj):
        """ Link to job outputs api endpoint """
        return reverse(viewname='waves_api_v1:waves-jobs-outputs', request=self.context['request'],
                       kwargs={'slug': obj.slug})

    def get_inputs(self, obj):
        """ Link to job inputs api endpoint """
        return reverse(viewname='waves_api_v1:waves-jobs-inputs', request=self.context['request'],
                       kwargs={'slug': obj.slug})
