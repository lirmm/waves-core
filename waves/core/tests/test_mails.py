...
from waves.core.models import Job, JobInput, JobOutput, Service, Submission
...
class JobMailTest(WavesBaseTestCase):
    ...
    def test_mail_job(self):
        ...
        job.status = waves.core.adaptors.const.JOB_COMPLETED
        # job.save()
        job.check_send_mail()
        # no more mails
        self.assertEqual(len(mail.outbox), 1)

        job.status = waves.core.adaptors.const.JOB_TERMINATED
        # job.save()
        job.check_send_mail()
        ...
        logger.debug('Mail from: %s', sent_mail.from_email)
        logger.debug('Mail content: \n%s', sent_mail.body)
        job.status = waves.core.adaptors.const.JOB_ERROR
        # job.save()
        job.check_send_mail()
