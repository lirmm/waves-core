...
class TestJobRunner(WavesBaseTestCase):
    ...
    def tearDown(self):
        ...
        # if not waves.core.settings.WAVES_TEST_DEBUG:
    ...
    def testWrongJobStates(self):
        ...
        self.current_job.status = waves.core.adaptors.const.JOB_RUNNING
        ...
        self.current_job.status = waves.core.adaptors.const.JOB_UNDEFINED
        with self.assertRaises(JobInconsistentStateError):
            self.current_job.run_launch()
        self._debug_job_state()
        logger.debug('Test Cancel')
        self.current_job.status = waves.core.adaptors.const.JOB_COMPLETED
        with self.assertRaises(JobInconsistentStateError):
            self.current_job.run_cancel()
            # self.adaptor.cancel_job(self.current_job)
        logger.debug('Internal state %s, current %s', self.current_job._status, self.current_job.status)
        # status hasn't changed
        self.assertEqual(self.current_job.status, waves.core.adaptors.const.JOB_COMPLETED)
        ...
        self.current_job.status = waves.core.adaptors.const.JOB_RUNNING
        self.current_job.run_cancel()
        self.assertTrue(self.current_job.status == waves.core.adaptors.const.JOB_CANCELLED)
