...
class TestJobWorkflowMixin(object):
    def run_job_workflow(self, job):
        ...
        job.run_prepare()
        time.sleep(3)
        self.assertTrue(job.status == waves.core.adaptors.const.JOB_PREPARED,
                        "Wrong job status %s (expected %s) " % (job.status, waves.core.adaptors.const.JOB_PREPARED))
        job.run_launch()
        time.sleep(3)
        logger.debug('Remote Job ID %s', job.remote_job_id)
        self.assertTrue(job.status == waves.core.adaptors.const.JOB_QUEUED,
                        "Wrong job status %s (expected %s) " % (job.status, waves.core.adaptors.const.JOB_QUEUED))
        for ix in range(100):
            job_state = job.run_status()
            logger.info(u'Current job state (%i) : %s ', ix, job.get_status_display())
            if job_state >= waves.core.adaptors.const.JOB_COMPLETED:
                logger.info('Job state ended to %s ', job.get_status_display())
                if job_state == waves.core.adaptors.const.JOB_ERROR:
                    self.fail('Job is in error')
                break
            time.sleep(3)
        if job.status in (waves.core.adaptors.const.JOB_COMPLETED, waves.core.adaptors.const.JOB_TERMINATED):
            # Get job run details
            job.run_details()
            time.sleep(3)
            ...
            self.assertTrue(job.status == waves.core.adaptors.const.JOB_TERMINATED)
        else:
