...
from waves.core.management.runner import DaemonRunner
...
class JobQueueCommand(DaemonCommand):
    ...
    def loop_callback(self):
        ...
        jobs = Job.objects.prefetch_related('job_inputs'). \
            prefetch_related('outputs').filter(status__lt=waves.core.adaptors.const.JOB_TERMINATED)
        if jobs.count() > 0:
        ...
        for job in jobs:
            ...
            try:
                ...
                if job.status == waves.core.adaptors.const.JOB_CREATED:
                    job.run_prepare()
                    # runner.prepare_job(job=job)
                    logger.debug("[PrepareJob] %s (adaptor:%s)", job, job.adaptor)
                elif job.status == waves.core.adaptors.const.JOB_PREPARED:
                    logger.debug("[LaunchJob] %s (adaptor:%s)", job, job.adaptor)
                    job.run_launch()
                    # runner.run_job(job)
                elif job.status == waves.core.adaptors.const.JOB_COMPLETED:
