...
from waves.core.adaptors.exceptions import AdaptorJobStateException
...
class JobAdaptor(object):
    ...
    def prepare_job(self, job):
        ...
        """
        try:
            assert (job.status <= waves.core.adaptors.const.JOB_CREATED)
        except AssertionError:
            raise AdaptorJobStateException(job.status, waves.core.adaptors.const.JOB_CREATED)
        self.connect()
    ...
    def run_job(self, job):
        ...
        :raise: :class:`waves.core.adaptors.exceptions.RunnerNotReady` if adaptor is not initialized
        ...
        """
        try:
            assert (job.status == waves.core.adaptors.const.JOB_PREPARED)
        except AssertionError:
            raise AdaptorJobStateException(job.status, waves.core.adaptors.const.JOB_PREPARED)
        self.connect()
        self._run_job(job)
        job.status = waves.core.adaptors.const.JOB_QUEUED
        return job
    ...
    def cancel_job(self, job):
        ...
        """
        try:
            assert (job.status <= waves.core.adaptors.const.JOB_SUSPENDED)
        except AssertionError:
            raise AdaptorJobStateException(job.status, waves.core.adaptors.const.STATUS_MAP[0:5])
        self.connect()
        self._cancel_job(job)
        job.status = waves.core.adaptors.const.JOB_CANCELLED
        return job
    ...
    def job_status(self, job):
        ...
        logger.debug('Current remote state %s mapped to %s', self._job_status(job),
                     waves.core.adaptors.const.STATUS_MAP.get(job.status, 'Undefined'))
        return job
