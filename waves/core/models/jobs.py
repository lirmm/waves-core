...
from waves.core.adaptors.const import JobRunDetails
from waves.core.exceptions import WavesException
from waves.core.exceptions.jobs import *
from waves.core.mails import JobMailer
...
from waves.core.models.submissions import Submission, SubmissionOutput
from waves.core.settings import waves_settings
from waves.core.utils import normalize_value
...
class JobManager(models.Manager):
    ...
    def create_from_submission(self, submission, submitted_inputs, email_to=None, user=None):
        ...
        :rtype: :class:`waves.core.models.jobs.Job`
        """
        try:
...
class Job(TimeStamped, Slugged, UrlMixin):
    ...
    _status = models.IntegerField('Job status', choices=waves.core.adaptors.const.STATUS_LIST,
                                  default=waves.core.adaptors.const.JOB_CREATED)
    ...
    def adaptor(self):
        ...
        :rtype: `waves.core.adaptors.runner.JobRunnerAdaptor`
        """
        if self._adaptor:
    ...
    def label_class(self):
        ...
        :rtype: unicode
        """
        if self.status in (waves.core.adaptors.const.JOB_UNDEFINED, waves.core.adaptors.const.JOB_SUSPENDED):
            return 'warning'
        elif self.status == waves.core.adaptors.const.JOB_ERROR:
            return 'danger'
        elif self.status == waves.core.adaptors.const.JOB_CANCELLED:
            return 'info'
        else:
            return 'success'
    ...
    def check_send_mail(self):
        ...
        """
        mailer = JobMailer()
        if self.status != self.status_mail and self.status == waves.core.adaptors.const.JOB_ERROR:
        ...
        if waves_settings.NOTIFY_RESULTS and self.notify:
            if self.email_to is not None and self.status != self.status_mail:
                ...
                try:
                    nb_sent = 0
                    if self.status == waves.core.adaptors.const.JOB_CREATED:
                        nb_sent = mailer.send_job_submission_mail(self)
                    elif self.status == waves.core.adaptors.const.JOB_TERMINATED:
    ...
    def error(self, message):
        ...
        self.status = waves.core.adaptors.const.JOB_ERROR
    ...
    def _run_action(self, action):
        ...
        if action == 'prepare_job':
            status_allowed = waves.core.adaptors.const.STATUS_LIST[1:2]
        elif action == 'run_job':
            status_allowed = waves.core.adaptors.const.STATUS_LIST[2:3]
        elif action == 'cancel_job':
            # Report fails to a AdaptorException raise during cancel process
            status_allowed = waves.core.adaptors.const.STATUS_LIST[1:6]
            if getattr(self.adaptor, 'state_allow_cancel', None):
                status_allowed = self.adaptor.state_allow_cancel
        elif action == 'job_results':
            status_allowed = waves.core.adaptors.const.STATUS_LIST[6:7] + waves.core.adaptors.const.STATUS_LIST[9:]
        elif action == 'job_run_details':
            status_allowed = waves.core.adaptors.const.STATUS_LIST[6:10]
        else:
            # By default let all status allowed
            status_allowed = waves.core.adaptors.const.STATUS_LIST
    ...
    def next_status(self):
        ...
        if self.status in self.NEXT_STATUS:
            ...
            return waves.core.adaptors.const.JOB_UNDEFINED

    def run_prepare(self):
        """ Ask job adaptor to prepare run (manage input files essentially) """
        self._run_action('prepare_job')
        self.status = waves.core.adaptors.const.JOB_PREPARED

    def run_launch(self):
        """ Ask job adaptor to actually launch job """
        self._run_action('run_job')
        self.status = waves.core.adaptors.const.JOB_QUEUED

    def run_status(self):
        """ Ask job adaptor current job status """
        self._run_action('job_status')
        logger.debug('job current state :%s', self.status)
        if self.status == waves.core.adaptors.const.JOB_COMPLETED:
            self.run_results()
        if self.status == waves.core.adaptors.const.JOB_UNDEFINED and self.nb_retry > waves_settings.JOBS_MAX_RETRY:
            self.run_cancel()
        self.save()
        return self.status
    ...
    def run_cancel(self):
        ...
        self.status = waves.core.adaptors.const.JOB_CANCELLED
    ...
    def run_results(self):
        ...
        if self.exit_code != 0:
            ...
            self.status = waves.core.adaptors.const.JOB_TERMINATED
    ...
    def allow_rerun(self):
        ...
        return self.status not in (waves.core.adaptors.const.JOB_CREATED, waves.core.adaptors.const.JOB_UNDEFINED)
    ...
    def re_run(self):
        ...
        self.status = waves.core.adaptors.const.JOB_CREATED
    ...
    def default_run_details(self):
        ...
        prepared = self.job_history.filter(status=waves.core.adaptors.const.JOB_PREPARED).first()
        finished = self.job_history.filter(status__gte=waves.core.adaptors.const.JOB_COMPLETED).first()
...
class JobInputManager(models.Manager):
    ...
    def create_from_submission(self, job, service_input, order, submitted_input):
        ...
        :rtype: :class:`waves.core.models.jobs.JobInput`
        """
...
class JobInput(Ordered, Slugged, ApiModel):
    ...
    #: Reference to related :class:`waves.core.models.services.SubmissionParam`
